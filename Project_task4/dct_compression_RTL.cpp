...
void dct_compression_RTL::process_RTL()
{
	sc_int<9>	temp_out;
		...
		switch (state.read())
		{
			...
			case READVALUE: // read value
			{
				...
				if (ready_i.read() == true)
				{
					...
					i = count_i.read();
					j = count_j.read();
					temp_out = static_cast < sc_int<9> >(floor(value3/quantization[(i<<3)+j]+0.5));
			...
			case ZZREAD: //zig zag read
			{
				...
				else
				{
					...
					{
						value2 = temp_block[(j<<3)+i].read();
						block[k].write(value2);
						value2 = temp_block[((7-j)<<3)+(7-i)].read();
