...
SC_MODULE(jpeg_enc_float)
{
	...
	dct_compression_RTL_float						dct_compression_1;

	jpeg_enc_float(sc_module_name _name, sc_ufixed<14,-2,SC_TRN,SC_WRAP>* _quantization_i, int _maxwidth):
		sc_module(_name),
		r2b_out("r2b_out",1),
		dct_out("dct_out",1),
		...
		dct_1("dct_1"),
		FF2PC_1("FF2PC_1"),
		dct_compression_1("dct_compression_1",_quantization_i)
	{
		gen_reset_1.clk(clk);
